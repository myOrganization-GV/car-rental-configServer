pipeline {
    ...
    environment {
        APP_NAME = "car-rental-config-server"
        RELEASE = "1.0.0"
    ...
    stages {
        ...
        stage("Checkout Config Server") {
            steps {
                git branch: 'main', credentialsId: 'GITHUB_LOGIN', url: 'https://github.com/myOrganization-GV/car-rental-configServer'
            }
        }
        stage("Build Config Server Application") {
            steps {
        ...
        stage("Build and Push Docker Image") {
            steps {
                script {
                    def jarPath = "target/car-rental-configServer-0.0.1-SNAPSHOT.jar"
            ...
            steps {
                ...
                withCredentials([file(credentialsId: 'GCP_SA_KEY', variable: 'KEYFILE')]) {
                    sh '''
                        ...
                        --zone=${GCP_ZONE} --quiet \\
                        --command "cd ~/car-rental-app && docker compose pull car-rental-config-server && docker compose up -d --no-deps --force-recreate $APP_NAME"
                    '''
                }
            }
        }
